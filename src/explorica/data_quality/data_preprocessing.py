...
def get_missing(
    ...
    validate_unique_column_names(
        df,
        err_msg=_errors["duplicate_keys_f"].format("data", "data"),
    )
...
def drop_missing(
    ...
    """
    df = convert_dataframe(data)
    validate_unique_column_names(df, _errors["duplicate_keys_f"].format("data", "data"))
    if axis not in (0, 1):
    ...
    result_df = df.copy()
    nans = get_missing(result_df)
    nan_pct = nans["pct_of_nans"]
    if axis == 0:
        # Identify columns where NaN proportion is under the threshold
        cols_to_remove = nan_pct[(nan_pct < threshold) & (nan_pct != 0)].index.to_list()
...
def get_constant_features(
    ...
    validate_string_flag(
        method,
        supported_methods,
        _errors["unsupported_method_f"].format(method, set(supported_methods)),
    )
    if df.empty:
...
def get_categorical_features(
    ...
    **kwargs,
    ...
    def _filter_bin_const(df):
        ...
        result["bin_cols"].extend([col for col in df.columns if df[col].nunique() == 2])
    ...
    def _filter_categories(df, threshold, included_dtypes, **kwargs):
        ...
        if params["sign_bin"]:
            result["is_binary"] = {
                col: 1 if col in bin_const_cols["bin_cols"] else 0 for col in df.columns
            }
    ...
    df = convert_dataframe(data)
    validate_unique_column_names(df, err_msg=_errors["duplicate_keys_f"])
