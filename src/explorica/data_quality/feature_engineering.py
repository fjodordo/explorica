...
def ordinal_encode(
    ...
    **kwargs,
    ...
    def encode(
        ...
        get_order_methods = {
            "frequency": lambda: data.value_counts().sort_values(ascending=asc).index,
            "alphabetical": lambda: data.value_counts().sort_index(ascending=asc).index,
    ...
    validate_string_flag(
        order_method,
        supported_order,
        _errors["unsupported_method_f"].format(order_method, supported_order),
    )
    if order_method in {"mean", "median", "mode"}:
        ...
        validate_array_not_contains_nan(
            df_order_by,
            err_msg=_errors["array_contains_nans_f"].format("order_by"),
        )
        validate_lengths_match(
            df,
            df_order_by,
            err_msg=_errors["arrays_lens_mismatch_f"].format("data", "order_by"),
        )
    # row wise
    if axis == 1:
        encoded = encode(
...
def discretize_continuous(
    ...
    def encode(
        ...
        if how == "uniform":
            ...
            for col in data.columns:
                ...
                labels_by_col = labels[col] if labels is not None else None
                encoded[col] = pd.qcut(data[col], q=bins_by_col, labels=labels_by_col)
    ...
    params = _discretize_continuous_transform_input_parameters(
    ...
    _discretize_continuous_validate_input_parameters(
        df,
        bins,
...
def _discretize_continuous_validate_input_parameters(
    ...
    **kwargs,
    ...
    validate_string_flag(
        ...
        _errors["unsupported_method_f"].format(
    ...
    validate_array_not_contains_nan(
        data, _errors["array_contains_nans_f"].format("data")
    )
    if labels is not None:
        for col in list(labels):
            if any(pd.isna(labels[col])):
                raise ValueError(_errors["array_contains_nans_f"].format("intervals"))
    for bins_number in bins.values():
        if not isinstance(bins_number, (np.integer, int)) or bins_number <= 0:
            raise ValueError(
                _errors["is_not_positive_integer_f"].format(
...
def _discretize_continuous_transform_input_parameters(
    ...
    def transform_labels(data: pd.DataFrame, labels):
        ...
        if isinstance(labels, pd.Series):
            ...
            if (
                ...
                labels2d = {col: list(lbls) for col, lbls in zip(data.columns, labels)}
            else:
    ...
    interval_format = convert_from_alias(interval_format, supported["interval_formats"])
    binning_method = convert_from_alias(binning_method, supported["binning_methods"])
    output = {
        "interval_format": interval_format,
        "binning_method": binning_method,
        "labels": labels2d,
